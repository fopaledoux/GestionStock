...
Class Master extends DBConnection {
	...
	function save_supplier(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Ce fournisseur existe deja.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			$resp['status'] = 'success';
			if(empty($id)){
				$res['msg'] = "Nouveau fournisseur enregistré.";
	...
	function delete_supplier(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Fornisseur supprimé avec succès.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_item(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "L'article existe déjà sous le fournisseur sélectionné.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"Article enregistré avec succès.");
			else
				$this->settings->set_flashdata('success',"Article mis à jour avec succès.");
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error."[{$sql}]";
		}
	...
	function delete_item(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Article supprimé avec succès.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
		...
		if($save){
			...
			if(!empty($data)){
				...
				if(!$save){
					...
					$resp['msg'] = 'Le bon de commande n\'a pas pu être enregistré. Erreur: '.$this->conn->error;
					$resp['sql'] = "INSERT INTO `po_items` (`po_id`,`item_id`,`quantity`,`price`,`unit`,`total`) VALUES {$data}";
				}
			}
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = 'Une erreur s\'est produite. Error: '.$this->conn->error;
		}
		if($resp['status'] == 'success'){
			if(empty($id)){
				$this->settings->set_flashdata('success'," Le nouveau bon de commande a été créé avec succès.");
			}else{
				$this->settings->set_flashdata('success'," Détails du bon de commande mis à jour avec succès.");
			}
		}
	...
	function delete_po(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Bon de commande details Supprimé avec succès.");
			if($bo->num_rows > 0){
	...
	function save_receiving(){
		...
		if($save){
			...
			$resp['status'] = 'failed';
			$resp['msg'] = 'Une erreur s\'est produite. Erreur: '.$this->conn->error;
		}
		if($resp['status'] == 'success'){
			if(empty($id)){
				$this->settings->set_flashdata('success'," Le nouveau stock a été reçu avec succès.");
			}else{
				$this->settings->set_flashdata('success'," Détails du stock reçu Mis à jour avec succès.");
			}
		}
	...
	function delete_receiving(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Les détails de la commande reçue ont été supprimés avec succès.");
	...
	function delete_bo(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Details Supprimé avec succès.");
	...
	function save_return(){
		...
		if($resp['status'] == 'success'){
			if(empty($id)){
				$this->settings->set_flashdata('success'," Le nouvel enregistrement d'article retourné a été créé avec succès.");
			}else{
				$this->settings->set_flashdata('success'," L'enregistrement de l'article retourné a été mis à jour avec succès.");
			}
		}
	...
	function delete_return(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Les enregistrements d'articles retournés ont été supprimés avec succès.");
			if(isset($res)){
				$this->conn->query("DELETE FROM `stock_list` where id in ({$res['stock_ids']})");
			}
		}else{
	...
	function save_sale(){
		...
		if($resp['status'] == 'success'){
			if(empty($id)){
				$this->settings->set_flashdata('success',"Le nouveau dossier de vente a été créé avec succès.");
			}else{
				$this->settings->set_flashdata('success'," Le dossier de vente a été mis à jour avec succès.");
			}
		}
	...
	function delete_sale(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Les enregistrements de vente ont été supprimés avec succès.");
			if(isset($res)){
				$this->conn->query("DELETE FROM `stock_list` where id in ({$res['stock_ids']})");
			}
		}else{

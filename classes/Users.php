...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		if(empty($id)){
			...
			if($qry){
				...
				$this->settings->set_flashdata('success','Les détails de l\'utilisateur ont été enregistrés avec succès.');
				$resp['status'] = 1;
			}else{
				$resp['status'] = 2;
			}
	...
	public function save_susers(){
		...
		foreach($_POST as $k => $v){
			...
			if($save){
			$this->settings->set_flashdata('success','Les détails de l\'utilisateur ont été mis à jour avec succès.');
			foreach($_POST as $k => $v){
				if(!in_array($k,array('id','password'))){
					if(!empty($data)) $data .=" , ";
					$this->settings->set_userdata($k,$v);
